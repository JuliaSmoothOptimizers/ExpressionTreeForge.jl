@testset "ADNLPModel model" begin
  ...
  m = Model()
  @variable(m, x[1:n])
  @NLobjective(m, Min, sum(x[i]^2 - x[i] for i = 1:n))
  ...
  y = ones(n)
  @test M_evaluation_expr_tree.evaluate_expr_tree(expr_tree, y) ==
        M_evaluation_expr_tree.evaluate_expr_tree(expr_tree_jump, y)
end
