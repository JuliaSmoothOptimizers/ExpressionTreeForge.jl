@testset "general tree" begin
  ...
  @NLobjective(
    m,
    Min,
    tan(x[1]) * 1 / x[3] + exp(x[2]) - cos(cos(x[7])^2) +
    cos(-cos(cos(x[7])^2)) +
    sin(cos(cos(x[7])^2)) +
    tan(cos(cos(x[7])^2)) +
    sin(-cos(cos(x[7])^2)) +
    ...
    4 +
    cos(x[3]^2) +
    1 / 4 * sin(x[4]) - x[5]^2 * sin(exp(x[6])) / (7 + x[7]^2.5) +
    (-x[6]^2)^2 +
    5^3 +
    x[6]^0 +
    x[3]^1 +
...
@testset "create_node_expr" begin
  ...
  # Constants
  constantF64 = [3, 5.0]
  constantF32 = Vector{Float32}([3, 5.0])

  @test create_node_expr.(constantF64) != create_node_expr.(constantF32)
  @test create_node_expr.(constantF64) ==
        map(constant -> create_node_expr(constant, x), constantF64)

  # Variables
  variables = [(:x, 1), (:x, 2)]
  x = ones(2)
  xdic = Dict{Int, Float64}(1 => 1.0, 2 => 1.0)
  @test map(variable -> create_node_expr(variable...), variables) ==
        map(variable -> create_node_expr(variable..., x), variables)
  var = create_node_expr(variables[1]...)
  @test M_interface_expr_node._evaluate_node(var, x) ==
        M_interface_expr_node._evaluate_node(var, xdic)
