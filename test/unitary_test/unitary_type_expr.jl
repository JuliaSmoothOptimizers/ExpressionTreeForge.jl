...
using CalculusTreeTools.implementation_type_expr,
  CalculusTreeTools.implementation_convexity_type, CalculusTreeTools.implementation_convexity_type
...
@testset "test implementation_type_expr" begin
  ...
  @test (x -> implementation_type_expr._type_product(linear, x)).(all_types) ==
        [linear, quadratic, cubic, more, more]
  @test (x -> implementation_type_expr._type_product(quadratic, x)).(all_types) ==
        [quadratic, cubic, more, more, more]
  @test (x -> implementation_type_expr._type_product(cubic, x)).(all_types) ==
        [cubic, more, more, more, more]
  @test (x -> implementation_type_expr._type_product(cubic, x)).(all_types) ==
        [cubic, more, more, more, more]
  @test (x -> implementation_type_expr._type_product(more, x)).(all_types) ==
        [more, more, more, more, more]

  # test de _type_power
  @test (x -> implementation_type_expr._type_power(0, x)).(all_types) ==
        [constante, constante, constante, constante, constante]
  @test (x -> implementation_type_expr._type_power(1, x)).(all_types) == all_types
  @test (x -> implementation_type_expr._type_power(2, x)).(all_types) ==
        [constante, quadratic, more, more, more]
  @test (x -> implementation_type_expr._type_power(3, x)).(all_types) ==
        [constante, cubic, more, more, more]
  @test (x -> implementation_type_expr._type_power(4, x)).(all_types) ==
  ...
  @test (x -> trait_type_expr.type_product(linear, x)).(all_types) ==
        [linear, quadratic, cubic, more, more]
  @test (x -> trait_type_expr.type_product(quadratic, x)).(all_types) ==
        [quadratic, cubic, more, more, more]
  ...
  # test de _type_power
  @test (x -> trait_type_expr.type_power(0, x)).(all_types) ==
        [constante, constante, constante, constante, constante]
  @test (x -> trait_type_expr.type_power(1, x)).(all_types) == all_types
  @test (x -> trait_type_expr.type_power(2, x)).(all_types) ==
        [constante, quadratic, more, more, more]
...
@testset "Convexity" begin
  ...
  @test (x -> implementation_convexity_type.is_treated(x)).(all_status) ==
        [false, true, true, true, true, true]
  @test (x -> implementation_convexity_type.is_not_treated(x)).(all_status) ==
        [true, false, false, false, false, false]
  @test (x -> implementation_convexity_type.is_constant(x)).(all_status) ==
        [false, true, false, false, false, false]
  @test (x -> implementation_convexity_type.is_linear(x)).(all_status) ==
        [false, true, true, false, false, false]
  @test (x -> implementation_convexity_type.is_convex(x)).(all_status) ==
        [false, true, true, true, false, false]
  @test (x -> implementation_convexity_type.is_concave(x)).(all_status) ==
        [false, true, true, false, true, false]
  @test (x -> implementation_convexity_type.is_unknown(x)).(all_status) ==
  ...
  @test (x -> implementation_convexity_type.is_treated(x)).(all_status_wrapper) ==
        [false, true, true, true, true, true]
  @test (x -> implementation_convexity_type.is_not_treated(x)).(all_status_wrapper) ==
        [true, false, false, false, false, false]
  @test (x -> implementation_convexity_type.is_constant(x)).(all_status_wrapper) ==
        [false, true, false, false, false, false]
  @test (x -> implementation_convexity_type.is_linear(x)).(all_status_wrapper) ==
        [false, true, true, false, false, false]
  @test (x -> implementation_convexity_type.is_convex(x)).(all_status_wrapper) ==
        [false, true, true, true, false, false]
  @test (x -> implementation_convexity_type.is_concave(x)).(all_status_wrapper) ==
        [false, true, true, false, true, false]
  @test (x -> implementation_convexity_type.is_unknown(x)).(all_status_wrapper) ==
        [false, false, false, false, false, true]

  @test (x -> implementation_convexity_type.get_convexity_wrapper(x)).(all_status_wrapper) ==
        all_status
