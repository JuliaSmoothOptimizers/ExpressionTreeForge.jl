...
function _node_convexity(
  ...
  elseif ((bs - bi <= π) && (sin(bi) >= 0) && (sin(bs) >= 0)) && (
    implementation_convexity_type.is_linear(status) ||
    (implementation_convexity_type.is_convex(status) && cos(bi) <= 0 && cos(bs <= 0)) ||
    (implementation_convexity_type.is_concave(status) && cos(bi) >= 0 && cos(bs >= 0))
  )
...
@inline function _evaluate_node!(
  ...
  abstract_expr_node.set_myRef!(ref, sin(value_ch[1]))
end
