module plus_operators
    ...
    @inline _evaluate_node!(op :: plus_operator, value_ch :: AbstractVector{abstract_expr_node.myRef{T}}, ref :: abstract_expr_node.myRef{T} ) where T <: Number = @fastmath length(ref) > 1 ? abstract_expr_node.set_myRef!(ref, sum(value_ch)) : abstract_expr_node.set_myRef!(ref, get_myRef(value_ch[1]))
