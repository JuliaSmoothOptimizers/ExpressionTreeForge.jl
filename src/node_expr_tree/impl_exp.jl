module exp_operators
    ...
    @inline function _evaluate_node!(op :: exp_operator, value_ch :: AbstractVector{myRef{Y}}, ref :: abstract_expr_node.myRef{Y}) where Y <: Number
        length(value_ch) == 1 || error("exp has more than one argument")
        abstract_expr_node.set_myRef!(ref, exp(value_ch[1]) )
    end

    @inline function _evaluate_node!(op :: exp_operator, vec_value_ch :: Vector{Vector{myRef{Y}}}, vec_ref :: Vector{abstract_expr_node.myRef{Y}}) where Y <: Number
